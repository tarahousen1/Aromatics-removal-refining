...
from functions.hydrocracking_functions import hydrogen_consumption
from functions.hydrocracking_functions import compute_hydrogen_consumption_iterative
import os
...
if aromatics_removal_technique == 'hydrocracking':
    ...
    hydro_cracking_outputs['Volume %']['C5 to 180°F'] = interpolate_naphtha_vol(hydrocracking_feed_api_gravity, h2_input, hydro_cracking_feed_Kw)
    ...
    hydro_cracking_inputs['lb/hr']['Hydrogen'] = total_hydro_cracking_inputs_bpcd * h2_input * 0.00521 / 24 + hydro_cracking_outputs['lb/hr H2']['H2S']
...
if aromatics_removal_technique == 'hydrotreatment':
    ...
    hydrotreatment_outputs['wt% monoaromatics']['HT Kerosine (330-480°F)'] = 0
    hydrotreatment_outputs['vol% monoaromatics']['HT Kerosine (330-480°F)'] = 0
    hydrotreatment_outputs['wt% polyaromatics']['HT Kerosine (330-480°F)'] = 0
    hydrotreatment_outputs['vol% polyaromatics']['HT Kerosine (330-480°F)'] = 0
    ...
    hydrotreatment_outputs['lb/hr S']['H2S'] = pd.to_numeric(pd.Series(hydrotreatment_inputs['lb/hr S'].values()), errors='coerce').fillna(0).sum()
    
    MW_S = 32.06 # g/mol
    MW_H2S = 34.08 # g/mol
    MW_H2 = 2.02 # g/mol
    hydrotreatment_outputs['lb/hr']['H2S'] = hydrotreatment_outputs['lb/hr S']['H2S'] * MW_H2S / MW_S
    hydrotreatment_outputs['lb/hr H2']['H2S'] = hydrotreatment_outputs['lb/hr']['H2S'] * MW_H2 / MW_H2S

    # Hydrogen balance for hydrotreatment
    hydrogen_balance = {}
    hydrogen_balance['lb/hr'] = {}
    hydrogen_balance['scf/bbl'] = {}

    hydrogen_balance['lb/hr']['Sulfur removal'] = hydrotreatment_outputs['lb/hr H2']['H2S']
    hydrogen_balance['scf/bbl']['Sulfur removal'] = hydrogen_balance['lb/hr']['Sulfur removal'] * 24 / conversion_parameters['H2_scf_to_lb'] / total_hydrotreatment_inputs_bpcd

    hydrogen_balance['lb/hr']['Saturation after desulfurization'] = hydrotreatment_outputs['lb/hr H2']['H2S']
    hydrogen_balance['scf/bbl']['Saturation after desulfurization'] = hydrogen_balance['lb/hr']['Saturation after desulfurization'] * 24 / conversion_parameters['H2_scf_to_lb'] / total_hydrotreatment_inputs_bpcd
    
    if hydrotreatment_inputs['wppm N']['Kerosine (330-480°F)'] is None or (isinstance(hydrotreatment_inputs['wppm N']['Kerosine (330-480°F)'], float) and math.isnan(hydrotreatment_inputs['wppm N']['Kerosine (330-480°F)'])):
        hydrotreatment_inputs['wppm N']['Kerosine (330-480°F)'] = 0

    scf_H2_per_perc_N_reduction = 320   # scf/bbl/wt% change (57 Nm3/m3/wt% change)
    change_in_N_perc = total_hydrotreatment_inputs_wppm_nitrogen /10000
    hydrogen_balance['scf/bbl']['Denitrogenation'] = scf_H2_per_perc_N_reduction * change_in_N_perc
    hydrogen_balance['lb/hr']['Denitrogenation'] = hydrogen_balance['scf/bbl']['Denitrogenation'] * total_hydrotreatment_inputs_bpcd / 24 * conversion_parameters['H2_scf_to_lb']
    ...
    hydrotreatment_outputs['wt% H2']['C3 and lighter'] = (wt_perc_H2_CH4 * hydrotreatment_outputs_wt_perc_CH4 + wt_perc_H2_C2H6 * hydrotreatment_outputs_wt_perc_C2H6  + wt_perc_H2_C3H8 * hydrotreatment_outputs_wt_perc_C3H8) / (hydrotreatment_outputs_wt_perc_CH4 + hydrotreatment_outputs_wt_perc_C2H6 + hydrotreatment_outputs_wt_perc_C3H8)
    hydrotreatment_outputs['wt% H2']['iC4'] = wt_perc_H2_iC4
    hydrotreatment_outputs['wt% H2']['nC4'] = wt_perc_H2_nC4

    wt_perc_HC_gas = 0.002  # PRELIM assumption
    
    hydrotreatment_outputs['lb/hr']['HC gas'] = hydrotreatment_inputs['lb/hr']['Kerosine (330-480°F)'] * wt_perc_HC_gas    # lb/hr 

    hydrotreatment_outputs['lb/hr']['total gas flow']  = hydrotreatment_outputs['lb/hr']['H2S'] + hydrotreatment_outputs['lb/hr']['HC gas']
    ...
    H2_for_carbon_displacement = hydrotreatment_outputs['lb/hr']['HC gas'] * hydrotreatment_outputs['wt% H2']['HC gas'] / 100

    hydrotreatment_outputs['lb/hr H2']['HC gas'] = hydrotreatment_outputs['wt% H2']['HC gas'] * hydrotreatment_outputs['lb/hr']['HC gas'] /100

    hydrotreatment_outputs['lb/hr H2']['total gas flow'] = hydrotreatment_outputs['lb/hr H2']['HC gas'] + hydrotreatment_outputs['lb/hr H2']['H2S']
    hydrotreatment_outputs['wt% H2']['total gas flow'] = 100 * hydrotreatment_outputs['lb/hr H2']['total gas flow']/ hydrotreatment_outputs['lb/hr']['total gas flow'] 

    #for outer_key in hydrogen_balance:
    #    for inner_key in hydrogen_balance[outer_key]:
    #        value = hydrogen_balance[outer_key][inner_key]
    #        if value is None or (isinstance(value, float) and math.isnan(value)):
    #            hydrogen_balance[outer_key][inner_key] = 0

    #if refinery_type == 'hydroskimming':
    #    wt_perc_H2_kerosine_calc_method_1 = 0.9015 * hydrotreatment_inputs['wt% H2']['Kerosine (330-480°F)'] + 1.557

    #    wt_perc_H2_kerosine_calc_method_2 = 100 * ( hydrotreatment_inputs['lb/hr H2']['Kerosine (330-480°F)'] + hydrogen_balance['lb/hr']['Sulfur removal'] + hydrogen_balance['lb/hr']['Saturation after desulfurization'] + hydrogen_balance['lb/hr']['Denitrogenation'] + H2_for_carbon_displacement - hydrotreatment_outputs['lb/hr H2']['total gas flow'] ) / (hydrotreatment_inputs['lb/hr']['Kerosine (330-480°F)'] + hydrogen_balance['lb/hr']['Sulfur removal'] + hydrogen_balance['lb/hr']['Saturation after desulfurization'] + hydrogen_balance['lb/hr']['Denitrogenation'] + H2_for_carbon_displacement - ( hydrotreatment_inputs['lb/hr']['Kerosine (330-480°F)'] * (hydrotreatment_inputs['wppm N']['Kerosine (330-480°F)'] - hydrotreatment_outputs['wppm N']['HT Kerosine (330-480°F)']) / 1000000 - hydrotreatment_outputs['lb/hr']['total gas flow']))

    #    if (wt_perc_H2_kerosine_calc_method_1 - hydrotreatment_inputs['wt% H2']['Kerosine (330-480°F)']) > (wt_perc_H2_kerosine_calc_method_2 - hydrotreatment_inputs['wt% H2']['Kerosine (330-480°F)']):
    #        hydrotreatment_outputs['wt% H2']['HT Kerosine (330-480°F)'] = wt_perc_H2_kerosine_calc_method_1
    #    else:
    #        hydrotreatment_outputs['wt% H2']['HT Kerosine (330-480°F)'] = wt_perc_H2_kerosine_calc_method_2

    #elif refinery_type == 'medium conversion with gas oil hydrocracking':
    #    hydrotreatment_outputs['wt% H2']['HT Kerosine (330-480°F)'] = 0.7867 * hydrotreatment_inputs['wt% H2']['Kerosine (330-480°F)'] + 3.2402

    #C_displaced_by_H2 = hydrotreatment_outputs['lb/hr']['HC gas'] * (1 - hydrotreatment_outputs['wt% H2']['HC gas'] / 100)    # lb/hr 
    #C_liq = hydrotreatment_inputs['lb/hr']['Kerosine (330-480°F)'] * (1 - hydrotreatment_inputs['wt% S']['Kerosine (330-480°F)']/100 - hydrotreatment_inputs['wt% H2']['Kerosine (330-480°F)']/100 - hydrotreatment_inputs['wppm N']['Kerosine (330-480°F)']/1000000) - C_displaced_by_H2
    
   # hydrotreatment_outputs['lb/hr']['HT Kerosine (330-480°F)'] = C_liq / (1 - (hydrotreatment_outputs['wt% S']['HT Kerosine (330-480°F)']/100) -(hydrotreatment_outputs['wt% H2']['HT Kerosine (330-480°F)']/100))

    #hydrotreatment_outputs['lb/hr H2']['HT Kerosine (330-480°F)'] = hydrotreatment_outputs['wt% H2']['HT Kerosine (330-480°F)'] * hydrotreatment_outputs['lb/hr']['HT Kerosine (330-480°F)'] / 100

    #hydrogen_balance['lb/hr']['Additional H2 to meet liquid product specs'] = hydrotreatment_outputs['lb/hr H2']['HT Kerosine (330-480°F)'] - hydrotreatment_inputs['lb/hr H2']['Kerosine (330-480°F)'] - hydrogen_balance['lb/hr']['Sulfur removal'] - hydrogen_balance['lb/hr']['Denitrogenation']
    #hydrogen_balance['scf/bbl']['Additional H2 to meet liquid product specs'] = hydrogen_balance['lb/hr']['Additional H2 to meet liquid product specs'] * 24 / conversion_parameters['H2_scf_to_lb'] / total_hydrotreatment_inputs_bpcd

    #hydrogen_balance['lb/hr']['Hydrogen for carbon displacement'] = hydrotreatment_outputs['lb/hr H2']['HC gas']
    #hydrogen_balance['scf/bbl']['Hydrogen for carbon displacement'] = hydrogen_balance['lb/hr']['Hydrogen for carbon displacement'] * 24 / conversion_parameters['H2_scf_to_lb'] / total_hydrotreatment_inputs_bpcd
    ...
    hydrotreatment_outputs['lb/hr']['C3 and lighter'] = hydrotreatment_outputs['lb/hr']['HC gas'] * (hydrotreatment_outputs_wt_perc_CH4 + hydrotreatment_outputs_wt_perc_C2H6 + hydrotreatment_outputs_wt_perc_C3H8)
    hydrotreatment_outputs['lb/hr']['iC4'] = hydrotreatment_outputs['lb/hr']['HC gas'] * hydrotreatment_outputs_wt_perc_iC4
    hydrotreatment_outputs['lb/hr']['nC4'] = hydrotreatment_outputs['lb/hr']['HC gas'] * hydrotreatment_outputs_wt_perc_nC4

    hydrotreatment_outputs['lb/hr H2']['C3 and lighter'] = hydrotreatment_outputs['wt% H2']['C3 and lighter'] * hydrotreatment_outputs['lb/hr']['C3 and lighter']
    hydrotreatment_outputs['lb/hr H2']['iC4'] = hydrotreatment_outputs['wt% H2']['iC4'] * hydrotreatment_outputs['lb/hr']['iC4']
    hydrotreatment_outputs['lb/hr H2']['nC4'] = hydrotreatment_outputs['wt% H2']['nC4'] * hydrotreatment_outputs['lb/hr']['nC4']
    ...
    gas_output_lb_per_hr = hydrotreatment_outputs['lb/hr']['C3 and lighter'] + hydrotreatment_outputs['lb/hr']['iC4'] + hydrotreatment_outputs['lb/hr']['nC4']
    gas_output_BPCD = hydrotreatment_outputs['BPCD']['C3 and lighter'] + hydrotreatment_outputs['BPCD']['iC4'] + hydrotreatment_outputs['BPCD']['nC4']
    Hydrogen_content_gas_output = hydrotreatment_outputs['lb/hr H2']['C3 and lighter'] + hydrotreatment_outputs['lb/hr H2']['iC4'] + hydrotreatment_outputs['lb/hr H2']['nC4']

    # Chemical hydrogen consumption
    H_HDS, H_HDN, H_HDA, H_chem_total, H_chem_scf_per_bbl, hydrotreatment_inputs['lb/hr']['Hydrogen'], hydrotreatment_outputs['lb/hr']['HT Kerosine (330-480°F)'], hydrotreatment_outputs['BPCD']['HT Kerosine (330-480°F)'], iterations= compute_hydrogen_consumption_iterative(
    ...
    hydrotreatment_inputs['lb/hr']['Kerosine (330-480°F)'], gas_output_lb_per_hr, gas_output_BPCD, 
    hydrotreatment_outputs['lb/hr from bbl/day']['HT Kerosine (330-480°F)'], total_hydrotreatment_inputs_bpcd,
    Hydrogen_content_gas_output, tolerance=1e-6, max_iter=100
    )   

    print(f"HDS: {H_HDS:.3f} Nm³ H₂/m³ oil")
    print(f"HDN: {H_HDN:.3f} Nm³ H₂/m³ oil")
    print(f"HDA: {H_HDA:.3f} Nm³ H₂/m³ oil")
    print(f"Total chemical H₂ consumption: {H_chem_total:.3f} Nm³ H₂/m³ oil")
    print(f"Total chemical H₂ consumption: {H_chem_scf_per_bbl:.3f} scf/bbl")

    print(f"Iterations to converge: {iterations}")

    hydrotreatment_outputs['Volume %']['C3 and lighter'] = hydrotreatment_outputs['BPCD']['C3 and lighter'] / (hydrotreatment_outputs['BPCD']['C3 and lighter'] + hydrotreatment_outputs['BPCD']['iC4'] + hydrotreatment_outputs['BPCD']['nC4'] + hydrotreatment_outputs['BPCD']['HT Kerosine (330-480°F)'])
    hydrotreatment_outputs['Volume %']['iC4'] = hydrotreatment_outputs['BPCD']['iC4'] / ( hydrotreatment_outputs['BPCD']['C3 and lighter'] + hydrotreatment_outputs['BPCD']['iC4'] + hydrotreatment_outputs['BPCD']['nC4'] + hydrotreatment_outputs['BPCD']['HT Kerosine (330-480°F)'])
    hydrotreatment_outputs['Volume %']['nC4'] = hydrotreatment_outputs['BPCD']['nC4'] / ( hydrotreatment_outputs['BPCD']['C3 and lighter'] + hydrotreatment_outputs['BPCD']['iC4'] + hydrotreatment_outputs['BPCD']['nC4'] + hydrotreatment_outputs['BPCD']['HT Kerosine (330-480°F)'])
    ...
    H_chem_total_scf_per_bbl = H_chem_total / conversion_parameters['H2_scf_to_Nm3'] * conversion_parameters['bbl_to_m3']
    H_chem_total_lb_per_hr = H_chem_total_scf_per_bbl * total_hydrotreatment_inputs_bpcd / 24 * conversion_parameters['H2_scf_to_lb']

    H_added_to_molecules_lb_per_hr = H_chem_total_lb_per_hr - hydrotreatment_outputs['lb/hr H2']['H2S']
    hydrotreatment_outputs['lb/hr H2']['HT Kerosine (330-480°F)'] = H_added_to_molecules_lb_per_hr + hydrotreatment_inputs['lb/hr H2']['Kerosine (330-480°F)']

    hydrotreatment_outputs['wt% H2']['HT Kerosine (330-480°F)'] = 100 * hydrotreatment_outputs['lb/hr H2']['HT Kerosine (330-480°F)']/hydrotreatment_outputs['lb/hr']['HT Kerosine (330-480°F)']
